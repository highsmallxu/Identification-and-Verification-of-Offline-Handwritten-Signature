...
function [assign,a] = kmeans(a,kmax,maxit,init,fid)
  ...
  a=set(a,'labels',ones(m,1),'lablist',[1:kmax]','prior',[]); % for speed
    ...
    elseif (size(init,1) == 1) & strcmp(init,'kmeans++')
        clusters = kmeanspp(+a,kmax);
        w = nmc(dataset(clusters,[1:kmax]'));
...
function clusters = kmeanspp(X, K)

    % Initialize variables and perform precomputations
    [N, D] = size(X);
    sum_X2 = sum(X .^ 2, 2);
    clusters = nan(K, D);
    clusters_ind = nan(1, K);
    DD = nan(N, K);
    
    % Loop over clusters
    for k=1:K
        
        % Pick cluster randomly
        if k == 1
            clusters_ind(k) = randi(N);
        else
            min_DD = min(DD(:,1:k - 1), [], 2);
            clusters_ind(k) = find(cumsum(min_DD ./ sum(min_DD)) > rand, 1, 'first');
        end
        clusters(k,:) = X(clusters_ind(k),:);    
        
        % Compute distances to cluster
        DD(:,k) = sum_X2(clusters_ind(k)) + sum_X2 - 2 .* (X * clusters(k,:)');
    end
end
